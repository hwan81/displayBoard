...
**📋 상세한 설치 가이드는 [INSTALL.md](INSTALL.md) 파일을 참고하세요.**

### 빠른 설치 (요약)

#### XAMPP 로컬 설치
1. XAMPP 설치 및 Apache, MySQL 실행
2. 파일을 `C:\xampp\htdocs\digital-signage`에 복사
3. phpMyAdmin에서 데이터베이스 생성 및 `tables.sql` 실행
4. `php/db_config.php` 설정 파일 생성
5. `http://localhost/digital-signage` 접속

#### 웹 호스팅 설치
1. PHP 7.4+, MySQL 지원 호스팅 선택
2. FTP로 파일 업로드
3. 호스팅 관리 패널에서 데이터베이스 생성
4. `php/db_config.php` 설정 파일 수정
5. 도메인으로 접속
...
문제가 있거나 질문이 있으시면 [Issues](https://github.com/your-username/digital-signage-system/issues) 페이지에 등록해 주세요.
