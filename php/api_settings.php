...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    // 설정 정보 가져오기
    $stmt = $conn->prepare("SELECT api_area_code, api_school_code FROM users WHERE id = ?");
    ...
    $stmt = $conn->prepare("UPDATE users SET api_area_code = ?, api_school_code = ? WHERE id = ?");
    $stmt->bind_param("ssi", $area_code, $school_code, $user_id);
