...
$stmt = $conn->prepare("SELECT id, school_name, api_area_code, api_school_code, hide_meal_info FROM users WHERE username = ?");
...
if (!$user_info) {
    http_response_code(404);
    echo json_encode(['status' => 'error', 'message' => '사용자를 찾을 수 없습니다.']);
    $conn->close();
    exit;
}
...
echo json_encode([
    ...
    'user_info' => [
        ...
        'api_area_code' => $user_info['api_area_code'] ?? null,
        'api_school_code' => $user_info['api_school_code'] ?? null,
        'hide_meal_info' => $user_info['hide_meal_info'] ?? 0
    ],
    'slides' => $slides
]);
